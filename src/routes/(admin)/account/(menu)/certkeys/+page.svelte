<script lang="ts">
  ...
  const unsubscribe = connection.subscribe(async (c:Connection) => {
    ...
    if (c.GetProxyURL() !== "" && !once) {
      ...
      const certkeys = (await resp.json()).Data

      for (let certkey of certkeys) {
        AddCertKey(certkey);
      }
    }
  });
...
<div class="navbar bg-base-100">
  <div class="flex-1">
    <a href="/account/resources" class="btn btn-ghost text-xl">CertKeys</a>
  </div>
  <div class="flex-none">
...
<div class="overflow-x-auto">
  {#if Object.entries($certkeysMap).length == 0}
