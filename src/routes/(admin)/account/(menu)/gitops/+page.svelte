<script lang="ts">
  ...
  import {containersMap} from "../stores/containers"
...
<div class="navbar bg-base-100">
  <div class="flex-1">
    <a href="/account/gitops" class="btn btn-ghost text-xl">Gitops</a>
  </div>
  <div class="flex-none">
      ...
      {#if $gitopsMap[gitopsId].Definition != undefined}
        ...
        <td>
          ...
          <label class="btn btn-circle" on:click={() => triggerAction(gitopsId)}>
            ...
            <label id="label-swap-{$gitopsMap[gitopsId].Definition.meta.group}-{$gitopsMap[gitopsId].Definition.meta.name}" class="btn btn-circle swap swap-rotate">
            <input id="{$gitopsMap[gitopsId].Definition.meta.group}-{$gitopsMap[gitopsId].Definition.meta.name}-swap"
            ...
            <div class="border border-gray-300 rounded-lg p-4 pt-0">
              ...
              <div class="navbar bg-base-100 mt-2">
                <div class="navbar-start">
                  ...
                  <a class="text-xl">{$gitopsMap[gitopsId].Definition.meta.group}/{$gitopsMap[gitopsId].Definition.meta.name}</a>
                </div>
  ...
  <div class="modal-box max-h-[80vh] w-11/12 max-w-full">
    ...
    {#if $gitopsMap[$openItem] != undefined}
      <div role="tablist" class="tabs tabs-lifted">
        {#if $modalItem != ""}
        <input type="radio" name="modal_tabs" value="messages" bind:group={$selectedTabModal} on:change={() => handleTabChange("Messages")} role="tab" class="tab" aria-label="Messages" checked />
        ...
        <input type="radio" name="modal_tabs" role="tab" class="tab" aria-label="Raw" value="raw" bind:group={$selectedTabModal} on:change={() => handleTabChange("Raw")}/>
        <div role="tabpanel" class="tab-content bg-base-200 border-base-100 rounded-box p-6 max-h-[80vh]">
        ...
        {#each $gitopsMap[$openItem].Gitops.Pack.Definitions as definition}
          {#if `${definition.Definition.Definition.kind}-${definition.Definition.Definition.meta.group}-${definition.Definition.Definition.meta.name}` == $modalItem}
            ...
            <div role="tabpanel" class="tab-content bg-base-200 border-base-100 rounded-box p-6 max-h-[80vh]">
              <EditorDiffModule original={definition.Definition.Definition} diff={$diffItem} language="json" theme="vs-light" height="height: 80vh"/>
            </div>
          {/if}
        {/each}
        {/if}
