...
<div class="navbar bg-base-100">
  <div class="flex-1">
    <a class="btn btn-ghost text-xl" href="/account/gitops">Gitops</a>
  </div>
  <div class="flex-none">
            ...
            {#each $gitopsIds as gitopsId}
              {#if $gitopsMap[gitopsId].Definition != undefined}
                <tr
                  ...
                  <td>
                    {$gitopsMap[gitopsId].Gitops.Git.Repository}
                    ({convertToShortGitIdFromArray($gitopsMap[gitopsId].Gitops.Commit.Hash)})
                  </td>
                  <td>
        ...
        {#if $gitopsMap[$openItem] != undefined}
          <div role="tablist" class="tabs tabs-lifted">
            {#if $modalItem != ""}
              ...
              <input type="radio" name="modal_tabs" role="tab" class="tab" aria-label="Raw" value="raw"
              ...
              {#each $gitopsMap[$openItem].Gitops.Pack.Definitions as definition}
                ...
                {#if `${def.kind}-${def.meta.group}-${def.meta.name}` == $modalItem}
                  <input type="radio" name="modal_tabs" role="tab" class="tab" aria-label="Diff" value="diff"
