<script lang="ts">
  ...
  import {configurationsMap, AddConfiguration} from "../stores/configurations";
  ...
  adminSection.set("configurations")
  ...
  const unsubscribe = connection.subscribe(async (c:Connection) => {
    ...
    if (c.GetProxyURL() !== "" && !once) {
      ...
      const configurations = (await resp.json()).Data
...
<div class="overflow-x-auto">
  {#if Object.entries($configurationsMap).length == 0}
    <EmptyModule></EmptyModule>
  {:else}
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-5">
  {#each Object.entries($configurationsMap) as [key, configuration]}
  <div class="card bg-base-100 shadow-sm">
    <div class="card-body">
      <h2 class="card-title">
        {configuration.meta.group}/{configuration.meta.name}
      </h2>
      <div class="overflow-x-auto">
        <table class="table">
          <tbody>
          <tr>
            <th>Group</th>
            <td>{configuration.meta.group}</td>
          </tr>
          <tr>
            <th>Name</th>
            <td>{configuration.meta.name}</td>
          </tr>
          </tbody>
        </table>
      </div>
      <ul class="list bg-base-100 rounded-box shadow-md">
        ...
        {#each Object.entries(configuration.spec.data) as [key,value]}
          <li class="list-row">
            <div>
            ...
            <div>
              <div>{key}</div>
              <div class="text-xs uppercase font-semibold opacity-60">Configuration</div>
            </div>
