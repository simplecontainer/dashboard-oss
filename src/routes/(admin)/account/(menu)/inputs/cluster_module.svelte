<script lang="ts">
    import { enhance, applyAction } from "$app/forms"
    import { page } from "$app/stores"
    import type { SubmitFunction } from "@sveltejs/kit"
    import {writable} from "svelte/store";
    import {onMount} from "svelte";
    import toastStore from "../../toasts";
    import {Connection} from "../../types/context/connection";
    import {Context} from "../../types/context/context";
    import {Cluster} from "../../types/context/cluster";

    const fieldError = (liveForm: FormAccountUpdateResult, name: string) => {
        let errors = liveForm?.errorFields ?? []
        return errors.includes(name)
    }

    let fieldsStore = writable<Field[]>()
    let decrypted = writable(false)

    let loading = $state(false)
    let showSuccess = $state(false)

    let addCtx = writable("")
    let addKey = writable("")

    onMount(() => {
        fieldsStore.set(fields)

        const unsCtx = addCtx.subscribe((val) => {
            formDecrypt(val, $addKey)
        })

        const unsKey = addKey.subscribe((val) => {
            formDecrypt($addCtx, val)
        })
    })

    function formDecrypt(ctx: string, key: string){
        if (ctx != "" && key != "") {
            let c = new Connection({})
            c.Cluster = new Cluster({})
            c.Cluster.EncryptedContext = ctx


            c.Decrypt(key).then((context) => {
                if (document.getElementById("context").classList.contains("input-error")) {
                    document.getElementById("context").classList.toggle("input-error")
                }

                if (document.getElementById("key").classList.contains("input-error")) {
                    document.getElementById("key").classList.toggle("input-error")
                }

                fieldsStore.update(cfields => cfields.filter(f => f.id !== "key"));

                setTimeout(() => {
                    document.getElementById('name').value = c.Context.Name
                    document.getElementById('api').value = c.Context.API

                    decrypted.set(true)
                }, 100)
            }).catch((e) => {
                if (!document.getElementById("context").classList.contains("input-error")) {
                    document.getElementById("context").classList.toggle("input-error")
                }

                if (!document.getElementById("key").classList.contains("input-error")) {
                    document.getElementById("key").classList.toggle("input-error")
                }

                toastStore.addToast({message: e.message, type: 'error'});
                decrypted.set(false)
            })
        }
    }

    type Field = {
        inputType?: string // default is "text"
        id: string
        label?: string
        initialValue: string | boolean
        placeholder?: string
        maxlength?: number
        bind?: string
    }

    interface Props {
        // Module context
        editable?: boolean
        dangerous?: boolean
        title?: string
        message?: string
        fields: Field[]
        formTarget?: string
        successTitle?: string
        successBody?: string
        editButtonTitle?: string | null
        editLink?: string | null
        saveButtonTitle?: string
        returnLink: string
    }

    let {
        editable = false,
        dangerous = false,
        title = "",
        message = "",
        fields,
        formTarget = "",
        successTitle = "Success",
        successBody = "",
        editButtonTitle = null,
        editLink = null,
        saveButtonTitle = "Save",
    }: Props = $props()

    const handleSubmit: SubmitFunction = () => {
        loading = true

        return async ({ update, result }) => {
            if (!$decrypted) {
                toastStore.addToast({message: "provided context is invalid", type: 'error'});
                loading = false;
                return;
            }

            await update({ reset: false })
            await applyAction(result)

            loading = false

            if (result.type === "success") {
                showSuccess = true
            }
        }
    }
</script>

<div class="card p-6 pb-7 mt-8 max-w-xl flex flex-col md:flex-row shadow-sm">
    {#if title}
        <div class="text-xl font-bold mb-3 w-48 md:pr-8 flex-none">{title}</div>
    {/if}

    <div class="w-full min-w-48">
        {#if !showSuccess}
            {#if message}
                <div class="mb-6 {dangerous ? 'alert alert-warning' : ''}">
                    {#if dangerous}
                        <svg
                                xmlns="http://www.w3.org/2000/svg"
                                class="stroke-current shrink-0 h-6 w-6"
                                fill="none"
                                viewBox="0 0 24 24"
                        ><path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                stroke-width="2"
                                d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"
                        /></svg
                        >
                    {/if}

                    <span>{message}</span>
                </div>
            {/if}
            <form
                    class="form-widget flex flex-col"
                    method="POST"
                    action={formTarget}
                    use:enhance={handleSubmit}
            >
                {#each $fieldsStore as field}
                    {#if field.label}
                        <label for={field.id}>
                            <span class="text-sm text-gray-500">{field.label}</span>
                        </label>
                    {/if}
                    {#if editable}
                        {#if field.id == "context"}
                        <input
                                id={field.id}
                                name={field.id}
                                type={field.inputType ?? "text"}
                                disabled={!editable}
                                placeholder={field.placeholder ?? field.label ?? ""}
                                class="{fieldError($page?.form, field.id)
                ? 'input-error'
                : ''} input-sm mt-1 input input-bordered w-full max-w-xs mb-3 text-base py-4"
                                maxlength={field.maxlength ? field.maxlength : null}
                                bind:value={$addCtx}
                        />
                        {:else if field.id == "key"}
                            <input
                                    id={field.id}
                                    name={field.id}
                                    type={field.inputType ?? "text"}
                                    disabled={!editable}
                                    placeholder={field.placeholder ?? field.label ?? ""}
                                    class="{fieldError($page?.form, field.id)
                ? 'input-error'
                : ''} input-sm mt-1 input input-bordered w-full max-w-xs mb-3 text-base py-4"
                                    maxlength={field.maxlength ? field.maxlength : null}
                                    bind:value={$addKey}
                            />
                        {:else}
                            <input
                                    id={field.id}
                                    name={field.id}
                                    type={field.inputType ?? "text"}
                                    disabled={!editable}
                                    placeholder={field.placeholder ?? field.label ?? ""}
                                    class="{fieldError($page?.form, field.id)
                ? 'input-error'
                : ''} input-sm mt-1 input input-bordered w-full max-w-xs mb-3 text-base py-4"
                                    value={$page.form ? $page.form[field.id] : field.initialValue}
                                    maxlength={field.maxlength ? field.maxlength : null}
                            />
                        {/if}
                    {:else}
                        <div class="text-lg mb-3">{field.initialValue}</div>
                    {/if}
                {/each}

                {#if $page?.form?.errorMessage}
                    <p class="text-red-700 text-sm font-bold mt-1">
                        {$page?.form?.errorMessage}
                    </p>
                {/if}

                {#if editable}
                    <div>
                        <button
                                type="submit"
                                class="ml-auto btn btn-sm mt-3 min-w-[145px] {dangerous
                ? 'btn-error'
                : 'btn-primary btn-outline'}"
                                disabled={loading}
                        >
                            {#if loading}
                <span
                        class="loading loading-spinner loading-md align-middle mx-3"
                ></span>
                            {:else}
                                {saveButtonTitle}
                            {/if}
                        </button>
                    </div>
                {:else if editButtonTitle && editLink}
                    <!-- !editable -->
                    <a href={editLink} class="mt-1">
                        <button
                                class="btn btn-outline btn-sm {dangerous
                ? 'btn-error'
                : ''} min-w-[145px]"
                        >
                            {editButtonTitle}
                        </button>
                    </a>
                {/if}
            </form>
        {:else}
            <!-- showSuccess -->
            <div>
                <div class="text-l font-bold">{successTitle}</div>
                <div class="text-base">{successBody}</div>
            </div>
            <a href="../">
                <button class="btn btn-outline btn-sm mt-3 min-w-[145px]">
                    Show resource
                </button>
            </a>
        {/if}
    </div>
</div>

<div class="toast toast-end">
    {#each $toastStore as { id, message, type }}
        <div
                class="alert alert-{type}  cursor-pointer"
                on:click={() => toastStore.removeToast(id)}
        >
            {message}
        </div>
    {/each}
</div>