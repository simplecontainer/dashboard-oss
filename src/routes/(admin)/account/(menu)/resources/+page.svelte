<script lang="ts">
  ...
  import {resourcesMap, AddResource} from "../stores/resources";
  ...
  const unsubscribe = connection.subscribe(async (c:Connection) => {
    ...
    if (c.GetProxyURL() !== "" && !once) {
      ...
      const resources = (await resp.json()).Data
...
<div class="navbar bg-base-100">
  <div class="flex-1">
    <a href="/account/resources" class="text-xl font-bold pl-2">Resources</a>
  </div>
  <div class="flex-none">
...
{:else}
  ...
  {#if Object.entries($resourcesMap).length == 0}
    <EmptyModule></EmptyModule>
  {:else}
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-5">
  {#each Object.entries($resourcesMap) as [key, resource]}
