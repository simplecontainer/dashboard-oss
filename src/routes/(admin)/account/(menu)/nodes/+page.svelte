<script lang="ts">
  ...
  import { AddNode, nodesMap } from '../stores/nodes';
              ...
              <td>
                <b>
                  ...
                  {#if node.State?.Control?.Upgrading == "in_progress" }
                    (Upgrading started)
                  {:else}
                    (<a href="https://github.com/simplecontainer/smr/releases/tag/smr-{node.Version.Node}"
                        target="_blank" class="link">{node.Version.Node}</a>)
                  {/if}
                </b>
              </td>
              <td>
...
<dialog class="modal" id="confirm-drain-node-modal">
  <div class="modal-box">
...
<dialog class="modal" id="confirm-restart-node-modal">
  <div class="modal-box">
    <h3 class="text-lg font-bold">Please confirm action?</h3>
    <ConfirmationModal
